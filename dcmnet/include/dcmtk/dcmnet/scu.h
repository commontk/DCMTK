...
/** Class representing an array of MOVEResponse objects from a single
 *  C-MOVE session.
 */
class MOVEResponses : public FINDResponses
{
// Empty for the moment.
};

// Likewise just use the FIND architecture
class MOVEResponse : public FINDResponse
{
};
...
public:
  ...
  virtual OFCondition sendMOVERequest( T_ASC_PresentationContextID presContextID,
                                       DcmDataset* moveKeys,
                                       FINDResponses* responses);

  virtual OFCondition handleMOVEResponse ( Uint16 presContextID,
  ...
  /** Set SCU's port number at which to listen for incoming associations.
   *  @param port [in] The port to be used
   */
  ...
  /** Set store SCP's AETitle to be used in move
   *  @param moveAETtitle [in] The SCU's AETitle to be used
   */
  void setMoveDestinationAETitle(const OFString& moveDestinationAETtitle);
  ...
  /** Returns the SCP's (Move Destinations's) AETitle configured.
   *  @return The AETitle configured to be sent to.
   */  
  OFString getMoveDestinationAETitle() const;
...
private:
  ...
  /// AEtitle of the move target
  OFString m_moveDestinationAETitle;
